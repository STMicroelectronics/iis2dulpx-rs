...
fn main() -> ! {
    ...
    loop {
        ...
        let accel = sensor.xl_data_get(&md).unwrap();
        let temp = sensor.outt_data_get().unwrap();

        // Print Accel data
        writeln!(
            tx,
            "Acceleration [mg]: {:4.2}\t{:4.2}\t{:4.2}",
            accel.mg[0], accel.mg[1], accel.mg[2]
        )
        .unwrap();
        // Print Temp data
        writeln!(tx, "Temp [degC]: {:3.2}", temp.heat.deg_c).unwrap();
    }
}
